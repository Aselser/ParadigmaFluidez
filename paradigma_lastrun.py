...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\agust\\OneDrive\\Desktop\\CNC\\Paradigmas\\Python\\fluency\\paradigma_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    sound_3 = sound.Sound('resources/beep.wav', secs=0.5, stereo=True, hamming=True,
        name='sound_3')
    ...
    blank_2 = visual.TextStim(win=win, name='blank_2',
        ...
        languageStyle='LTR',
        depth=-3.0);
    interrupt_key = keyboard.Keyboard()
    ...
    pulses_trial_instructionsComponents = [pulses_instructions, key_resp_2]
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # if blank_4 is starting this frame...
            if blank_4.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if blank_4 is active this frame...
            if blank_4.status == STARTED:
                # update params
                pass
            
            # if blank_4 is stopping this frame...
            if blank_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blank_4.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    blank_4.tStop = t  # not accounting for scr refresh
                    blank_4.frameNStop = frameN  # exact frame index
                    # update status
                    blank_4.status = FINISHED
        ...
        continueRoutine = True
        # update component parameters for each repeat
        sound_3.setSound('resources/beep.wav', secs=0.5, hamming=True)
        sound_3.setVolume(1.0, log=False)
        sound_3.seek(0)
        # Run 'Begin Routine' code from code_10
        recorder._send_pulse_to_arduino()
        # keep track of which components have finished
        pulsesComponents = [sound_3]
    ...
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    pulses_responseComponents = [text_5, key_resp_3]
    ...
    while continueRoutine:
        ...
        if key_instrucciones.status == STARTED and not waitOnFlip:
            theseKeys = key_instrucciones.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
    ...
    for thisCategoria in categorias:
        ...
        while continueRoutine:
            ...
            if key_categorias.status == STARTED and not waitOnFlip:
                theseKeys = key_categorias.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        interrupt_key.keys = []
        interrupt_key.rt = []
        _interrupt_key_allKeys = []
        # keep track of which components have finished
        grabacion_con_estimuloComponents = [text_estimulo, sound_1, blank_2, interrupt_key]
        ...
        while continueRoutine and routineTimer.getTime() < 65.0:
            ...
            # *interrupt_key* updates
            waitOnFlip = False
            
            # if interrupt_key is starting this frame...
            if interrupt_key.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
                interrupt_key.frameNStart = frameN  # exact frame index
                interrupt_key.tStart = t  # local t and not account for scr refresh
                interrupt_key.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(interrupt_key, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'interrupt_key.started')
                # update status
                interrupt_key.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(interrupt_key.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(interrupt_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
            # if interrupt_key is stopping this frame...
            if interrupt_key.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > interrupt_key.tStartRefresh + 60-frameTolerance:
                    # keep track of stop time/frame for later
                    interrupt_key.tStop = t  # not accounting for scr refresh
                    interrupt_key.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'interrupt_key.stopped')
                    # update status
                    interrupt_key.status = FINISHED
                    interrupt_key.status = FINISHED
            if interrupt_key.status == STARTED and not waitOnFlip:
                theseKeys = interrupt_key.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
                _interrupt_key_allKeys.extend(theseKeys)
                if len(_interrupt_key_allKeys):
                    interrupt_key.keys = _interrupt_key_allKeys[-1].name  # just the last key pressed
                    interrupt_key.rt = _interrupt_key_allKeys[-1].rt
                    interrupt_key.duration = _interrupt_key_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
            if thisExp.status == FINISHED or endExpNow:
                endExperiment(thisExp, inputs=inputs, win=win)
        ...
        # check responses
        if interrupt_key.keys in ['', [], None]:  # No response was made
            interrupt_key.keys = None
        categorias.addData('interrupt_key.keys',interrupt_key.keys)
        if interrupt_key.keys != None:  # we had a response
            categorias.addData('interrupt_key.rt', interrupt_key.rt)
            categorias.addData('interrupt_key.duration', interrupt_key.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    while continueRoutine:
        ...
        if key_insultos.status == STARTED and not waitOnFlip:
            theseKeys = key_insultos.getKeys(keyList=['space','s'], ignoreKeys=None, waitRelease=False)
    ...
    for thisInsulto in insultos:
        ...
        if interrupt_key.keys in ['', [], None]:  # No response was made
            interrupt_key.keys = None
        insultos.addData('interrupt_key.keys',interrupt_key.keys)
        if interrupt_key.keys != None:  # we had a response
            insultos.addData('interrupt_key.rt', interrupt_key.rt)
            insultos.addData('interrupt_key.duration', interrupt_key.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    for thisLetra in letras:
        ...
        while continueRoutine:
            ...
            if key_letras.status == STARTED and not waitOnFlip:
                theseKeys = key_letras.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
        ...
        if interrupt_key.keys in ['', [], None]:  # No response was made
            interrupt_key.keys = None
        letras.addData('interrupt_key.keys',interrupt_key.keys)
        if interrupt_key.keys != None:  # we had a response
            letras.addData('interrupt_key.rt', interrupt_key.rt)
            letras.addData('interrupt_key.duration', interrupt_key.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    df.to_csv(thisExp.dataFileName + '_spanish.csv', sep=';', decimal=',', index=False)
